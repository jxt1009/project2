...
/**
 * this class represents a route on the map valid routes: have two non equal
 * stations are either horizontal or vertical origin station must be north or
 * west of destination
 * 
 * @author Lauren Egts
 */
...
// not sure if this is right but the route interface requires a list of
// model.Track
...
public class Route implements model.Route {
	...
	/**
	 * constructor creates a route
	 * 
	 * @param origin
	 *            the station the route originates from
	 * @param destination
	 *            the station the route will go to
	 * @param orientation
	 *            the orientation of the route, either vertical or horizontal
	 */
	public Route(student.Station origin, student.Station destination, Orientation orientation) {
		...
		/**
		 * check for invalid routes origin and destination stations are on same space
		 * origin is not north from destination origin is not west from destination if
		 * route is invalid, throws an invalid route exception
		 */
		...
		if (origin.collocated(destination)) { // case origin and destination are on same space
			...
			if (!(origin.getRow() == destination.getRow() && origin.getCol() < destination.getCol()
					&& ort.toString().equals("HORIZONTAL") && length >= 1)) {
			...
			if (!(origin.getRow() < destination.getRow() && origin.getCol() == destination.getCol()
					&& ort.toString().equals("VERTICAL") && length >= 1)) {
				// if proposed route doesn't meet the conditions for it to be valid
				try {
					throw new InvalidRouteException(origin, destination);
				} catch (InvalidRouteException e) {
		...
		} else { // i need an else statement because i had else ifs but this shouldn't do
					// anything and if it does run something is very wrong
	...
	/**
	 * calculates the route's point value based on its length 1 - 1 point 2 - 2
	 * points 3 - 4 points 4 - 7 points 5 - 10 points 6 - 15 points 7 (or more) - 5
	 * * (length - 3) points
	 * 
	 ...
	 * 
	 * Remember that two stations next to each other is not valid, test is likely skewed as it claims it should be true when it's false.
	 * 
	 * @param space
	 *            the space to check whether or not it is in the route
	 * @return true if the space is in the route's path, false otherwise
	 * 
	 * 
	 */
	public boolean includesCoordinate(Space space) {
		for (model.Track track : tracks) {
	...
	/**
	 * this method attempts to claim the route for a baron (claimant) if the route's
	 * current baron is UNCLAIMED the method will return true indicating that the
	 * claim was successful otherwise it will return false
	 */
	public boolean claim(Baron claimant) {
		if (baron.name().equals("UNCLAIMED")) {
			baron = claimant;
			for (model.Track track : tracks) {
				((student.Track) track).claim(claimant);
			}
			return true;
		}
		return false;
